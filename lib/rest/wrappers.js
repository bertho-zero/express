...
const allowedMethods = (service, methodMap) => {
  return Object.keys(methodMap)
    .filter(method => typeof service[method] === 'function')
    .map(method => methodMap[method].toUpperCase())
...
function getHandler (method, argsOrder) {
  const getArgs = makeArgsGetter(argsOrder);

  return (service, methodMap) => {
    return (req, res, next) => {
      const { query } = req;
      const route = omit(req.params, '__feathersId');

      res.setHeader('Allow', allowedMethods(service, methodMap).join(','));
...
module.exports = {
  find: getHandler('find', ['params']),
  get: getHandler('get', ['id', 'params']),
  create: getHandler('create', ['data', 'params']),
  update: getHandler('update', ['id', 'data', 'params']),
  patch: getHandler('patch', ['id', 'data', 'params']),
  remove: getHandler('remove', ['id', 'params']),
  getHandler
};
